...
// Predicting on the test data
  ...
  // Predicting on the test data
  torch::Tensor y_prediction_Test = model.forward(x_TestPart);

  // Save predicted y in to a file y_prediction.pt
  torch::save(y_prediction_Test, "y_prediction_Test.pt");


  for (int iteration = 0; iteration < rest; ++iteration) {
    if (iteration % 50 == 0) {
      std::cout << "y_true " << y_TestPart[iteration].item<float>() << ", y_predict " << y_prediction_Test[iteration].item<float>() << std::endl;
    }
  }
 ...
 for (int iteration = 0; iteration < rest; ++iteration) {
  ...
  // Save y_prediction_Test in CSV format
  std::ofstream y_pred("y_prediction_Test.csv");
  for (int i = 0; i < N_SAMPLES - splitSize; ++i) {
    y_pred << y_prediction_Test[i].item<float>() << "\n";
  }
  y_pred.close();
...
// Save x_TrainPart in CSV format
  ...
  // Save num_iterations in CSV format
  std::ofstream nbr_iterations("num_iterations.csv");
  nbr_iterations << num_iterations << "\n";
  nbr_iterations.close();
 
  // Save num_iterations in CSV format
  std::ofstream nbr_ratio("ratio.csv");
  nbr_ratio << ratio*100 << "\n";
  nbr_ratio.close();
